...
func (lq *LocationQuery) QueryReviews() *ReviewQuery {
...
func (lq *LocationQuery) QueryOwner() *OwnerQuery {
	query := (&OwnerClient{config: lq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (lq *LocationQuery) WithReviews(opts ...func(*ReviewQuery)) *LocationQuery {
	query := (&ReviewClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (lq *LocationQuery) WithOwner(opts ...func(*OwnerQuery)) *LocationQuery {
	query := (&OwnerClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withAddress; query != nil {
		if err := lq.loadAddress(ctx, query, nodes,
			func(n *Location) { n.Edges.Address = []*Address{} },
			func(n *Location, e *Address) { n.Edges.Address = append(n.Edges.Address, e) }); err != nil {
			return nil, err
		}
	}
	if query := lq.withReviews; query != nil {
		if err := lq.loadReviews(ctx, query, nodes,
			func(n *Location) { n.Edges.Reviews = []*Review{} },
			func(n *Location, e *Review) { n.Edges.Reviews = append(n.Edges.Reviews, e) }); err != nil {
			return nil, err
		}
	}
	if query := lq.withOwner; query != nil {
		if err := lq.loadOwner(ctx, query, nodes, nil,
			func(n *Location, e *Owner) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
...
func (lq *LocationQuery) loadAddress(ctx context.Context, query *AddressQuery, nodes []*Location, init func(*Location), assign func(*Location, *Address)) error {
	...
	query.Where(predicate.Address(func(s *sql.Selector) {
		s.Where(sql.InValues(location.AddressColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "location_address" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (lq *LocationQuery) loadReviews(ctx context.Context, query *ReviewQuery, nodes []*Location, init func(*Location), assign func(*Location, *Review)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(location.ReviewsTable)
		s.Join(joinT).On(s.C(review.FieldID), joinT.C(location.ReviewsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(location.ReviewsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(location.ReviewsPrimaryKey[1]))
...
func (lq *LocationQuery) loadOwner(ctx context.Context, query *OwnerQuery, nodes []*Location, init func(*Location), assign func(*Location, *Owner)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Location)
	for i := range nodes {
		if nodes[i].owner_locations == nil {
			continue
		}
		fk := *nodes[i].owner_locations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "owner_locations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
