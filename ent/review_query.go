...
func (rq *ReviewQuery) QueryLocation() *LocationQuery {
...
func (rq *ReviewQuery) WithLocation(opts ...func(*LocationQuery)) *ReviewQuery {
	query := (&LocationClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withLocation; query != nil {
		if err := rq.loadLocation(ctx, query, nodes,
			func(n *Review) { n.Edges.Location = []*Location{} },
			func(n *Review, e *Location) { n.Edges.Location = append(n.Edges.Location, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewQuery) loadLocation(ctx context.Context, query *LocationQuery, nodes []*Review, init func(*Review), assign func(*Review, *Location)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(review.LocationTable)
		s.Join(joinT).On(s.C(location.FieldID), joinT.C(review.LocationPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(review.LocationPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(review.LocationPrimaryKey[0]))
