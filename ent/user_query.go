...
func (uq *UserQuery) QueryOwner() *OwnerQuery {
	query := (&OwnerClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithOwner(opts ...func(*OwnerQuery)) *UserQuery {
	query := (&OwnerClient{config: uq.config}).Query()
...
func (uq *UserQuery) loadOwner(ctx context.Context, query *OwnerQuery, nodes []*User, init func(*User), assign func(*User, *Owner)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*User)
	for i := range nodes {
		if nodes[i].owner_user == nil {
			continue
		}
		fk := *nodes[i].owner_user
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "owner_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
